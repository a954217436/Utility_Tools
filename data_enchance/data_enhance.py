...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=0.5, max_rotation_angle=5, 
                crop_rate=1, shift_rate=0.5, change_light_rate=0.5,
    ...
    def _addNoise(self, img):
        ...
        return random_noise(img, mode='gaussian', clip=True,var=0.002)*255

    
    # 调整亮度
    def _changeLight(self, img):
        # random.seed(int(time.time()))
        flag = random.uniform(0.2, 0.8) #flag>1为调暗,小于1为调亮
...
if __name__ == '__main__':
    ...
    source_pic_root_path = '/home/lm/Sample_And_Labels/Annotate_data/dlb/dlb_imgs'
    source_xml_root_path = '/home/lm/Sample_And_Labels/Annotate_data/dlb/dlb_xmls'
    for parent, _, files in os.walk(source_pic_root_path):
        for file in files:
            ...
            coords_old = parse_xml(xml_path)
            coords = [coord[:4] for coord in coords_old]
            img = cv2.imread(pic_path)
            auged_img, auged_bboxes = dataAug.dataAugment(img, coords)
            imgsize=list(auged_img.shape)
            coords_new=coords_old
            for i in range(len(coords_new)):
                  coords_new[i][:4]=auged_bboxes[i]
            img_name_new=str(file)[:-4]+'_enforce_4'+'.jpg'
            cv2.imwrite('/home/lm/Sample_And_Labels/Annotate_data/dlb/dlb_imgs_enchance/'+img_name_new,auged_img)
            generate_xml(img_name_new,coords_new,imgsize,'/home/lm/Sample_And_Labels/Annotate_data/dlb/dlb_xmls_enchance')
